...
def compare_weights(w1, w2):
    norm_ratio = torch.norm(w1) / torch.norm(w2)
    cosine_sim = torch.nn.functional.cosine_similarity(w1.flatten(), w2.flatten(), dim=0)
    rel_mse = torch.nn.functional.mse_loss(w1, w2) / w2.norm()
    return norm_ratio, cosine_sim, rel_mse
...
def check_weight_update(model, input_extended, functional_gradient):
    # Closed form computation of the weight update
    normalization = input_extended.size(0) * input_extended.size(1)
    matrix_s = torch.einsum(
        'ijp,ijq->pq',
        input_extended,
        input_extended
    ) / normalization

    norm_ratio, cosine_sim, rel_mse = compare_weights(matrix_s, model.currently_updated_block.tensor_s())
    assert torch.allclose(
        matrix_s,
        model.currently_updated_block.tensor_s()
    ), f"Tensor S -- Norm Ratio: {norm_ratio: .6f}, Cosine Sim: {cosine_sim: .6f}, Rel MSE: {rel_mse: .6f}"

    matrix_m = torch.einsum(
        'ijp,ijq->pq',
        input_extended,
        functional_gradient
    ) / normalization
    norm_ratio, cosine_sim, rel_mse = compare_weights(matrix_m, model.currently_updated_block.tensor_m())
    assert torch.allclose(
        matrix_m,
        model.currently_updated_block.tensor_m()
    ), f"Tensor M -- Norm Ratio: {norm_ratio: .6f}, Cosine Sim: {cosine_sim: .6f}, Rel MSE: {rel_mse: .6f}"

    delta_raw = torch.linalg.solve(matrix_s, matrix_m).t()
    weight, bias = delta_raw[:, :-1], delta_raw[:, -1]

    norm_ratio, cosine_sim, rel_mse = compare_weights(weight, model.currently_updated_block.optimal_delta_layer.weight)
    assert torch.allclose(
        weight,
        model.currently_updated_block.optimal_delta_layer.weight
    ), f"Weight -- Norm Ratio: {norm_ratio: .6f}, Cosine Sim: {cosine_sim: .6f}, Rel MSE: {rel_mse: .6f}"

    norm_ratio, cosine_sim, rel_mse = compare_weights(bias, model.currently_updated_block.optimal_delta_layer.bias)
    assert torch.allclose(
        bias,
        model.currently_updated_block.optimal_delta_layer.bias
    ), f"Bias -- Norm Ratio: {norm_ratio: .6f}, Cosine Sim: {cosine_sim: .6f}, Rel MSE: {rel_mse: .6f}"


def compare_weight_updates(model_1, model_2, part='all'):
    if part in ['parameter', 'all']:
        delta_weight_1 = model_1.currently_updated_block.optimal_delta_layer.weight.clone()
        delta_bias_1 = model_1.currently_updated_block.optimal_delta_layer.bias.clone()

        delta_weight_2 = model_2.currently_updated_block.optimal_delta_layer.weight.clone()
        delta_bias_2 = model_2.currently_updated_block.optimal_delta_layer.bias.clone()

        norm_ratio_delta_weight, cosine_sim_delta_weight, rel_mse_delta_weight = compare_weights(delta_weight_1,
                                                                                                 delta_weight_2)
        norm_ratio_delta_bias, cosine_sim_delta_bias, rel_mse_delta_bias = compare_weights(delta_bias_1, delta_bias_2)
        print(
            f"Delta weight -- Norm Ratio: {norm_ratio_delta_weight: .6f}, Cosine Sim: {cosine_sim_delta_weight: .6f}, Rel MSE: {rel_mse_delta_weight: .6f}")
        print(
            f"Delta bias -- Norm Ratio: {norm_ratio_delta_bias: .6f}, Cosine Sim: {cosine_sim_delta_bias: .6f}, Rel MSE: {rel_mse_delta_bias: .6f}")

    if part in ['neuron', 'all']:
        alpha_1 = model_1.currently_updated_block.previous_module.extended_output_layer.weight.clone()
        alpha_bias_1 = model_1.currently_updated_block.previous_module.extended_output_layer.bias.clone()
        omega_1 = model_1.currently_updated_block.extended_input_layer.weight.clone()

        alpha_2 = model_2.currently_updated_block.previous_module.extended_output_layer.weight.clone()
        alpha_bias_2 = model_2.currently_updated_block.previous_module.extended_output_layer.bias.clone()
        omega_2 = model_2.currently_updated_block.extended_input_layer.weight.clone()

        # compare the weights
        norm_ratio_alpha, cosine_sim_alpha, rel_mse_alpha = compare_weights(alpha_1, alpha_2)
        norm_ratio_alpha_bias, cosine_sim_alpha_bias, rel_mse_alpha_bias = compare_weights(alpha_bias_1, alpha_bias_2)
        norm_ratio_omega, cosine_sim_omega, rel_mse_omega = compare_weights(omega_1, omega_2)
        print(
            f"Alpha -- Norm Ratio: {norm_ratio_alpha: .6f}, Cosine Sim: {cosine_sim_alpha: .6f}, Rel MSE: {rel_mse_alpha: .6f}")
        print(
            f"Alpha bias -- Norm Ratio: {norm_ratio_alpha_bias: .6f}, Cosine Sim: {cosine_sim_alpha_bias: .6f}, Rel MSE: {rel_mse_alpha_bias: .6f}")
        print(
            f"Omega -- Norm Ratio: {norm_ratio_omega: .6f}, Cosine Sim: {cosine_sim_omega: .6f}, Rel MSE: {rel_mse_omega: .6f}")
